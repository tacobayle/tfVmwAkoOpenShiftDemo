resource "vsphere_virtual_machine" "controller_dhcp" {
  ...
  name             = "${var.controller.basename}-${random_string.id.result}"
...
resource "null_resource" "wait_https_controller" {
  ...
  provisioner "local-exec" {
    command = "until $(curl --output /dev/null --silent --head -k https://${vsphere_virtual_machine.controller_dhcp[count.index].default_ip_address}); do echo 'Waiting for Avi Controller to be ready'; sleep 10 ; done"
  }
}